...
# Main template blueprint for dashboard
template_bp = Blueprint('templates', __name__, url_prefix='/dashboard/templates')

# API blueprint for template operations
template_api_bp = Blueprint('template_api', __name__, url_prefix='/api/templates')
...
@template_api_bp.route('/preview/<template_name>')
def preview_template(template_name):
    """Get template HTML with sample data - PUBLIC endpoint for preview"""
    try:
        # Sanitize template name
        template_name = template_name.replace('..', '').replace('/', '')
        
        # Build path to template
        template_path = os.path.join(current_app.root_path, 'templates', 'email_templates', f'{template_name}.html')
        ...
        logger.info(f"✅ Template loaded: {template_name} ({len(html)} bytes)")
        ...
        sample_data = {
            ...
            '{{article_1_excerpt}}': 'We\'re thrilled to announce our latest features that will help you grow your business faster than ever...',
            '{{article_1_url}}': '#',
            '{{article_2_title}}': 'Customer Success Story',
            '{{article_2_excerpt}}': 'See how Company X achieved 300% growth using our platform...',
            '{{article_2_url}}': '#',
            ...
            '{{offer_description}}': 'Get 50% off all products this weekend only! Use code SAVE50 at checkout.',
            '{{expiry_date}}': 'January 31, 2025',
            '{{promo_code}}': 'SAVE50',
            '{{cta_url}}': 'https://sendbaba.com',
            ...
            '{{event_description}}': 'Join us for a day of learning, networking, and innovation with industry leaders!',
            '{{rsvp_url}}': 'https://sendbaba.com',
            ...
            '{{tax}}': '$9.90',
            '{{total}}': '$108.90',
            '{{invoice_url}}': 'https://sendbaba.com/invoice',
            '{{support_email}}': 'support@sendbaba.com',
            
            # URLs
            '{{unsubscribe_url}}': 'https://sendbaba.com/unsubscribe',
            '{{action_url}}': 'https://sendbaba.com',
            '{{link_url}}': 'https://sendbaba.com',
        ...
        logger.info(f"✅ Template {template_name} processed successfully")
